  ...
  if (completed.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-20 text-center">
        ...
        <p className="text-gray-500 max-w-xs">Your past workouts will appear here. Time to hit the gym!</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4">
      ...
      {completed.map(session => (
        <div key={session.id} className="bg-white dark:bg-gray-900 rounded-3xl p-5 border border-gray-100 dark:border-gray-800 shadow-sm overflow-hidden">
          ...
          <div className="space-y-3">
            {session.exercises.map((ex, idx) => (
              <div key={idx} className="flex justify-between items-center text-sm">
                ...
                <span className="text-gray-400 font-bold">{ex.sets.length} Sets</span>
              </div>
            ))}
          </div>
        </div>
